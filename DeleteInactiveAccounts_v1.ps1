...
$OU = "OU=Inactive,OU=Mount Clear Primary School,DC=curric,DC=mount-clear-ps,DC=wan"
...
foreach ($User in $Users)
    ...
    If ($User.Enabled -eq $false)
        {
        $HomeFolder = Get-ADUser -Identity $User -Properties HomeDirectory | select -ExpandProperty HomeDirectory
